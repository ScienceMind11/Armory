...
import io.starfall.armory.ArmoryClient;
...
public class ItemRendererMixin {
	...
	private BakedModel armory$changeItemModel(BakedModel model, ItemStack stack, ModelTransformation.Mode renderMode, boolean leftHanded, MatrixStack matrices, VertexConsumerProvider vertexConsumers, int light, int overlay, BakedModel modelAgain) {
		...
		if(stack.getOrCreateSubNbt("display").getBoolean("isDevilsknife")) {
			if(isHeld) return models.getModelManager().getModel(ArmoryClient.DEVILSKNIFE_HAND);
			return models.getModelManager().getModel(ArmoryClient.DEVILSKNIFE);
		}
